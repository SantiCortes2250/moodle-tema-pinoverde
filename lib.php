 ...
 * @return void
 */
function theme_pinoverde_page_init(moodle_page $page) {
    global $CFG;
    $page->requires->js_call_amd('theme_pinoverde/theme', 'init');
}
 ...
 * @return string
 */
function theme_pinoverde_process_css($css, $theme) {
    global $OUTPUT, $CFG;
    $css = theme_pinoverde_pre_css_set_fontwww($css);
    // Set custom CSS.
    $customcss = $theme->settings->customcss;
    $css = theme_pinoverde_set_customcss($css , $customcss);
    return $css;
}
 ...
 * @return string $css
 */
function theme_pinoverde_set_customcss($css, $customcss) {
 ...
 * @return bool
 */
function theme_pinoverde_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = []) {
    static $theme;
    $bgimgs = ['footerbgimg', 'loginbg', 'mspotmedia'];

    if (empty($theme)) {
        $theme = theme_config::load('pinoverde');
    }
 ...
 * @return string
 */
function theme_pinoverde_pre_css_set_fontwww($css) {
    global $CFG;
    ...
    $tag = '[[setting:fontwww]]';
    $css = str_replace($tag, $themewww.'/pinoverde/fonts/', $css);
    return $css;
}
 ...
 * @return string
 */
function theme_pinoverde_set_fontwww() {
    global $CFG;
    if (empty($CFG->themewww)) {
        $themewww = $CFG->wwwroot."/theme";
    } else {
    ...
    $fontwww = '$fontwww: "'.$themewww.'/pinoverde/fonts/"'.";\n";
    return $fontwww;
}
 ...
 * @return type|string
 */
function theme_pinoverde_get_logo_url($type = 'header') {
    global $OUTPUT;
    static $theme;
    if (empty($theme)) {
        $theme = theme_config::load('pinoverde');
 ...
 * @return string
 */
function theme_pinoverde_get_setting($setting, $format = true) {
    global $CFG, $PAGE;
    ...
    return (isset($return)) ? theme_pinoverde_lang($return) : '';
}
 ...
 * @return string
 */
function theme_pinoverde_lang($key='') {
    $pos = strpos($key, 'lang:');
    if ($pos !== false) {
        list($l, $k) = explode(":", $key);
        if (get_string_manager()->string_exists($k, 'theme_pinoverde')) {
            $v = get_string($k, 'theme_pinoverde');
            return $v;
        } else {
            return $key;
        }
 ...
 * @return string
 */
function theme_pinoverde_get_main_scss_content($theme) {
    global $CFG;
    ...
    if ($filename == 'default.scss') {
        $scss .= file_get_contents($CFG->dirroot . '/theme/pinoverde/scss/preset/default.scss');
    } else if ($filename == 'eguru') {
        $scss .= file_get_contents($CFG->dirroot . '/theme/pinoverde/scss/preset/eguru.scss');
    } else if ($filename == 'klass') {
        $scss .= file_get_contents($CFG->dirroot . '/theme/pinoverde/scss/preset/klass.scss');
    } else if ($filename == 'enlightlite') {
        $scss .= file_get_contents($CFG->dirroot . '/theme/pinoverde/scss/preset/enlightlite.scss');
    } else if ($filename && ($presetfile = $fs->get_file($context->id, 'theme_pinoverde', 'preset', 0, '/', $filename))) {
 ...
 */
function theme_pinoverde_get_pre_scss($theme) {
    $scss = '';
    $helperobj = new theme_pinoverde\helper();
 ...
 * @return string
 */
function theme_pinoverde_get_extra_scss($theme) {
